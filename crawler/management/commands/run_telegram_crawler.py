...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            '--once',
            action='store_true',
            help='Sadece bir kez çalıştır, sürekli çalışma'
        )
    ...
    async def run_crawler(self):
        ...
        client = TelegramClient('telegramcrawler',api_id,api_hash)
    ...
    async def process_channel(self, client, dialog, parser, archiver):
        ...
        async for message in client.iter_messages(dialog, limit=2500):
            if not message.text:
                continue
            if (message.id in deleted_message_ids or 
                message.id in existing_message_ids):
                continue
            classification = await parser.classify_message(message.text)
            ...
            processed_count += 1
            logger.info(f"New message recorded: {message.id} - {channel.channel_name}")
            ...
            if github_urls:
                ...
                logger.debug(f"GitHub URL not found in message: {message.id}")
        ...
        for url in github_urls:
            try:
                ...
                try:
                    ...
                    logger.error(f"Archiving error could not be recorded: {db_error}")
